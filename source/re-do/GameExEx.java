...
public class GameExEx {
    ...
    public String convertToCoords(String initialPosition, String targetPosition) {
        ...
        String targetPositionY = "" + (8 - ((int) targetPosition.charAt(1) - '0'));
        ...
        if (colour == "white" && !isWhite(gameBoard.getPiece(y,x))) {
            for (int i = 0; i < tempArr.length; i++) {
                for (int j = 0; j < tempArr.length; j++) {
                    if (((y + tempArr[j] != 8) && (y + tempArr[j] != -1) && (x + tempArr[i] != 8) && (x + tempArr[i] != -1)) && (gameBoard.getPiece(y + tempArr[j], x + tempArr[i]) == PAWN)) {
                        coords = Integer.toString(x + tempArr[i]) + Integer.toString(y + tempArr[j]) + Integer.toString(x) + Integer.toString(y);
                        if (isMoveLegal(coords, colour)) {
                            result = true;
                        }
                    }
